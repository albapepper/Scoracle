...
async def build_player_widget_payload(sport: str, player_id: str, season: Optional[str]) -> Dict[str, Any]:
    ...
    if s == 'NBA':
        try:
            ...
            return _local_player_widget_payload(s, player_id, season_norm)

    # Other sports to be implemented incrementally here
    return _local_player_widget_payload(s, player_id, season_norm)


def _local_player_widget_payload(sport: str, player_id: str, season: Optional[str]) -> Dict[str, Any]:
    row = local_get_player_by_id(sport, int(player_id))
    if not row:
